...
def _sample_text(raw_text: str, max_chars: int = 8000) -> str:
    ...
    head_size = max_chars // 2; tail_size = max_chars // 4; sample_size = max_chars - head_size - tail_size
    ...
    if len(paragraphs) <= 3: sample = middle[:sample_size]
    else:
        step = max(1, len(paragraphs) // 5); sampled_paras = paragraphs[::step][:8]
        sample = '\n\n'.join(sampled_paras)
...
def generate_lesson_content(course_title: str, module_title: str, lesson_title: str) -> dict:
    prompt = f"""You are a JSON API. Output ONLY a raw JSON object. START with {{ and END with }}. No markdown, no reasoning, no explanation. Fields: content_markdown (detailed lesson 300-500 words, 2-3 ```python code examples), exercises (array of {{title, description, instructions, starter_code, solution_code, hints, points}}). NEVER use triple double quotes.
